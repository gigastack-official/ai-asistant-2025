...
import { ArrowLeft, User, Bell, MessageCircle, Check, Loader2 } from "lucide-react"
...
export default function SettingsPage() {
  ...
  const handleLinkTelegram = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || "Ошибка привязки Telegram")
    } finally {
        ...
        <Card className="border-0 shadow-lg bg-white/70 backdrop-blur-sm">
          ...
          <CardContent className="space-y-4">
            <div>
              ...
              <div className="mt-1 p-3 bg-slate-50 rounded-xl text-slate-600">{user.username || user.name}</div>
            </div>
            {user.email && (
              <div>
                <label className="text-sm font-medium text-slate-700">Email</label>
                <div className="mt-1 p-3 bg-slate-50 rounded-xl text-slate-600">{user.email}</div>
              </div>
            )}
          </CardContent>
        </Card>
            ...
            {!telegramLinked ? (
              <form onSubmit={handleLinkTelegram} className="space-y-4">
                <div>
                  ...
                  />
                  <p className="mt-2 text-xs text-slate-500">Введите ваш Telegram тег для получения уведомлений</p>
                </div>
              ...
              <div className="text-center py-4">
                ...
                <p className="text-slate-600 font-medium">Telegram успешно подключен!</p>
                <p className="text-sm text-slate-500 mt-1">Теперь вы будете получать уведомления в Telegram</p>
              </div>
            )}
          </CardContent>
        </Card>
          ...
          <CardHeader className="pb-4">
            <CardTitle className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500">Настройки уведомлений</p>
              </div>
            </CardTitle>
          </CardHeader>
          <CardContent>
